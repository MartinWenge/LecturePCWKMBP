...
private:
  ...
  VectorDouble3 calculateRee();
  //! function to calculate radius of gyration
  VectorDouble3 calculateRg();
...
bool AnalyzerReeAndRg<IngredientsType>::execute(){
  ...
  VectorDouble3 dummy(calculateRee());
  results.at(1).push_back(dummy.getX());
  results.at(2).push_back(dummy.getY());
  results.at(3).push_back(dummy.getZ());
  
  dummy=calculateRg();
  results.at(4).push_back(dummy.getX());
  results.at(5).push_back(dummy.getY());
  results.at(6).push_back(dummy.getZ());
...
template <class IngredientsType>
VectorDouble3 AnalyzerReeAndRg<IngredientsType>::calculateRee(){
  VectorDouble3 Ree(ingredients.getMolecules()[0]-ingredients.getMolecules()[ingredients.getMolecules().size()-1]);
  return VectorDouble3(Ree.getX()*Ree.getX(),Ree.getY()*Ree.getY(),Ree.getZ()*Ree.getZ());
}

// calculate squared radius of gyration
template <class IngredientsType>
VectorDouble3 AnalyzerReeAndRg<IngredientsType>::calculateRg(){
  ...
  return (VectorDouble3(x,y,z)/ingredients.getMolecules().size());
}
