...
int main(int argc, char* argv[])
{
  ...
  double adsorpX, adsorpY, adsorpZ;
  
  // hier werden die boost funktionen zum Einlesen der Kommadozeilenoptionen benutzt
  options_description desc{"Allowed options"};
  desc.add_options()
    ("help,h", "help message")
    ("filename,f", value<std::string>(&fileName)->default_value("adsorbConfig.bfm"), "output filename")
    ("chainlength,n", value<uint32_t>(&chainLength)->default_value(64), "number of monomers in chain")
    ("numChains,m", value<uint32_t>(&numChains)->default_value(1), "number of chains")
    ...
    ("adsorpX,X", value<double>(&adsorpX)->default_value(0.0), "Adsorption energy in x direction")
    ("adsorpY,Y", value<double>(&adsorpY)->default_value(0.0), "Adsorption energy in y direction")
    ("adsorpZ,Z", value<double>(&adsorpZ)->default_value(0.0), "Adsorption energy in z direction");
  ...
  /* +++++++++++++++++++++++++  setup system  +++++++++++++++++++++++++++++++ */
  //zuerst definieren wir die Feature Liste, hier nur zwei, sonst für X Features LOKI_TYPELIST_X(1, 2, ..., X) einfügen. mögliche Features: FeatureExcludedVolume, FeatureBoltzmann, FeatureWall, FeatureNNInteractions, FeatureAdsorption
  typedef LOKI_TYPELIST_4(FeatureMoleculesIO,FeatureAttributes,FeatureAdsorption,FeatureExcludedVolumeSc<FeatureLattice<bool> >) Features;
  ...
  if(adsorpX >0.000001){
    ...
    ingredients.setAdsorptionEnergyX(adsorpX);
  }else{
  ...
  if(adsorpY >0.000001){
    ...
    ingredients.setAdsorptionEnergyY(adsorpY);
  }else{
  ...
  if(adsorpZ >0.000001){
    ...
    ingredients.setAdsorptionEnergyZ(adsorpZ);
  }else{
  ...
  if(chainLength==1)
    taskManager.addUpdater(new UpdaterAddLinearChains<IngredientsType>(ingredients,numChains,chainLength,1,1), 0);
  else
    taskManager.addUpdater(new UpdaterAddLinearChains<IngredientsType>(ingredients,numChains,chainLength), 0);
