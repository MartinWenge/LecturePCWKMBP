...
int main(int argc, char* argv[])
{
  ...
  uint32_t numberOfSteps, timeStep, timeIntervall;
  ...
  desc.add_options()
    ...
    ("nstep,n", value<uint32_t>(&numberOfSteps)->default_value(64), "number of walk steps")
    ("timeSteps,t", value<uint32_t>(&timeStep)->default_value(2000), "number of simulation steps")
    ("deltaT,d", value<uint32_t>(&timeIntervall)->default_value(10000), "time intervall");
  ...
  taskManager.addUpdater(new UpdaterSimpleSimulator<IngredientsType,MoveLocalSc>(ingredients,timeIntervall));
  taskManager.addAnalyzer(new AnalyzerReeAndRg<IngredientsType>(ingredients,fileReeRg));
  taskManager.addAnalyzer(new AnalyzerAutoCorrelation<IngredientsType>(ingredients,fileCorr,timeIntervall));
  taskManager.addAnalyzer(new AnalyzerWriteBfmFile<IngredientsType>(fileconfig,ingredients));
  
  taskManager.initialize();
  taskManager.run(timeStep);
  taskManager.cleanup();
