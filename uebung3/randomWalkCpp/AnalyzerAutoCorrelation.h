...
void AnalyzerAutoCorrelation<IngredientsType>::cleanup(){
  ...
  //getCorrelations(); // or just uncomment me
}
...
void AnalyzerAutoCorrelation<IngredientsType>::getCorrelations(){
   ...
   double ReeAverage(0.0);
   for(uint32_t t=0;t<reeTimeline.size();t++){
     ReeAverage+=reeTimeline.at(t);
   }
   ReeAverage/=reeTimeline.size();
   
   // do the correltation
   for(uint32_t delta_t=0;delta_t<reeTimeline.size();delta_t++){
     correlations.at(0).push_back(delta_t*timeStep);
     correlations.at(1).push_back(reeTimeline.at(0)*reeTimeline.at(delta_t));
   }
